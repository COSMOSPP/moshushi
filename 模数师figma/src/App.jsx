...
function SectionTitle({ asset, children }) {
  return (
    <div className="section-title">
      <img src={`/assets/${asset}`} alt={children} />
    </div>
  )
}

function DashboardChrome({ type, src, alt, withSweep = false }) {
  return (
    <div className={`dashboard-chrome dashboard-chrome--${type}`}>
      <img className={`dashboard-${type}`} src={src} alt={alt} />
      {withSweep && <span className="dashboard-chrome__sweep" aria-hidden="true" />}
    </div>
  )
}

function MetricIcon({ asset, refreshKey }) {
  return (
    <div className="metric-orbit" aria-hidden="true">
      <img className="metric-orbit__art" src={`/assets/${asset}`} alt="" />
      <span className="metric-orbit__refresh" key={refreshKey} />
    </div>
  )
}

function Metrics({ metrics, refreshKey }) {
  return (
...
function LeftPanel({ metrics, refreshKey }) {
  return (
    <aside className="left-panel dashboard-panel">
      <section className="intro-block">
        ...
        <div className="academy-photo">
          <img src="/assets/academy.jpg" alt="玄武模数师学院展示区" />
        </div>
...
function DetailTrendChart({ moduleId, trend, accent }) {
  const width = 640
  const height = 190
  const paddingX = 24
  const chartTop = 24
  ...
  const areaPath = `M ${points[0].x} ${chartBottom} L ${polyline.replaceAll(',', ' ')} L ${points.at(-1).x} ${chartBottom} Z`
...
function DetailDistribution({ distribution }) {
  let cursor = 0
...
function DetailProgress({ progress, accent }) {
  return (
...
function DetailView({ module, onClose }) {
  return (
    <div className="detail-layer" onMouseDown={onClose} role="presentation">
      <article
        ...
        style={{ '--detail-accent': module.accent }}
      >
        <header className="detail-view__header">
          <img src={`/assets/${module.asset}`} alt="" />
          <div>
...
function FloatingBubbles({ items, onSelect }) {
  return (
...
function OrbitalParticles({ items }) {
  return (
  ...
  return (
    <div className="industry-panel">
      <div className="industry-donut">
        <img src="/assets/industry-ring.svg" alt="就业行业占比环形图" />
...
function DashboardFrame({ logicalWidth, logicalHeight }) {
    ...
    >
      ...
      <div className="dashboard-content">
        <DashboardChrome
          ...
          alt="模数师数字平台驾驶舱"
        />
        <img className="dashboard-side dashboard-side--left" src="/assets/side-left.svg" alt="" />
        <img className="dashboard-side dashboard-side--right" src="/assets/side-right.svg" alt="" />
        ...
        <div className="main-visual" aria-label="学员全生命周期管理四象限态势">
          <img
            ...
            src="/assets/main-center-background.png"
            alt="学员全生命周期管理中央圆环"
          />
          <CenterLighting />
